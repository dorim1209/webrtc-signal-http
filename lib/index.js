...
const logModule = require("../../../module/logModule");
const redisModule = require("../../../module/redisModule");
const utilModule = require("../../../module/utilModule");
    ...
    this._gc = setInterval(() => {
      ...
      stalePeers.forEach((id) => {
        client
          ...
          .then((peerType) => {
            ...
            if (peerType == "render") {
              client.hget(id, "renderId").then((renderId) => {
                ...
                client.del(id);
                logModule.info(`${id}'s data is removed (TimeoutPeerList)`);
              });
            }
          ...
          .then(() => {
            this.removePeer(id);
            logModule.info(`${id} is removed (TimeoutPeerList)`);
          });
      });
...
const sendPeerMessage = (srcId, destId, data) => {
  ...
  try {
    if (peerWaitResSent[destId] == false) {
      ...
      logModule.debug(`sendPeerMessage : ${srcId}, ${destId}, ${data}`);
    ...
    logModule.error(`sendPeerMessage ERR : ${error}`);
  }
};
...
function signalRouterCreator(opts) {
  ...
  router.get("/sign_in", async (req, res) => {
    ...
    logModule.info(`Sign In Remote Address : ${req.ip}`);
    ...
    if (peerType == "player") {
      ...
      logModule.info(`LOGIN SUCCESS  : ${userId}(${peerType})`);
      ...
      if (mobileXVersion) {
        ...
        logModule.debug(`${userId}'s geo info : ${geo}`);
      ...
      if (!loginResult) {
        logModule.info(`LOGIN FAIL : ${userId} password is wrong`);
        return res.status(400).end();
      }

      if (isLoggedIn[userId]) {
        logModule.info(`LOGIN FAIL : ${userId} is already logged in`);
        return res.status(400).end();
      }
      ...
      logModule.error(`UNDEFINED peerType : ${peerType}`);

      router.peerList.removePeer(peerId);

      logModule.info(`${peerId} is removed`);
    ...
    logModule.info(`/sign_in : ${peerName}(${peerType})`);
    peerWaitResSent[peerId] = true;
  ...
  router.get("/broadcast_ready", async (req, res) => {
    logModule.info(`/broadcast_ready : ${req.query.peer_id}`);
    let peerId = req.query.peer_id;
    let channelName = req.query.channel_name;
    logModule.debug(`channelName : ${channelName}`);
    const exist = await client.exists(peerId);

    if (exist == 0) {
      logModule.info(`${peerId} was removed`);
      return res.status(400).end();
    }
    ...
    if (reconId && renderId) {
      logModule.info(`allocated reconId : ${reconId}, renderId : ${renderId}`);
      ...
      logModule.info(`{ type: "no_available_resource"}`);
    }
  });
  ...
  router.get("/broadcast_start", async (req, res) => {
    ...
    logModule.info(`/broadcast_start : ${req.query.peer_id}(${userId})`);
    ...
    if (!mobileXVersion && !mobileXClientVersion) {
      ...
      try {
        ...
        logModule.error(`broadcast_start update peer ERR - ${renderId}(${renderPeerId}) :   ${error}`);
      }
        ...
        if (renderIdArr.length > 1) {
          for (let i = 1; i < renderIdArr.length; i++) {
            try {
              ...
              logModule.error(`broadcast_start update peer ERR - ${renderIdArr[i]}(${idPeerId[renderIdArr[i]]}) : ${error}`);
            }
          }
        }
      }
  ...
  router.get("/broadcast_cancel", async (req, res) => {
    logModule.info(`/broadcast_cancel : ${req.query.peer_id}`);
    let peerId = req.query.peer_id;
    const exist = await client.exists(peerId);
  ...
  router.get("/broadcast_stop", async (req, res) => {
    logModule.info(`/broadcast_stop : ${req.query.peer_id}`);
    ...
    if (!mobileXVersion && !mobileXClientVersion) {
      ...
      try {
        ...
        logModule.error(`broadcast_stop update peer ERR - ${renderId}(${renderPeerId}) : ${error}`);
      }
    }
  ...
  router.get("/get_renderid", async (req, res) => {
    ...
    logModule.info(`/get_renderid : ${peerId} -> ${toPeerId}`);
    ...
    if (allocateResult.msg == "starting") {
      ...
      logModule.debug(`allocateResult.msg == "starting" renderIdPlayerPeerId[${renderId}] : ${renderIdPlayerPeerId[renderId]}`);
      ...
      logModule.debug(`allocateResult.msg == "started" renderIdPlayerPeerId[${renderId}] : ${renderIdPlayerPeerId[renderId]}`);
    }
  ...
  router.post("/message", bodyParser.text(), bodyParser.urlencoded({ extended: false }), async (req, res) => {
    ...
    if (!peerIdData) {
      logModule.info(`${toPeerId} was removed`);
      return res.status(400).end();
    }
    ...
    let data = req.body;
    logModule.debug(`/message : ${peerId} (${peerType})-> ${toPeerId}`);
    ...
    if (peerType == "guest" || peerType == "player") {
      if (toPeerType == "render") {
        ...
        if (data == "BYE") {
          logModule.info(`${peerName} (${peerType}${peerId}) disconnects from ${renderId}`);
          ...
          if ((mobileXVersion || mobileXClientVersion) && renderIdPlayerPeerId[renderId]) {
            ...
            if (idx > -1) {
              if (renderIdPlayerPeerIdArr.length == 1) {
                delete renderIdPlayerPeerId[renderId];
                logModule.debug(`renderIdPlayerPeerId[${renderId}] is deleted`);
                rrModule.stopRender(renderId);
              } else {
                renderIdPlayerPeerId[renderId].splice(idx, 1);
                logModule.debug(`renderIdPlayerPeerId[${renderId}] : ${renderIdPlayerPeerId[renderId]}`);
              }
            }
          ...
          if (bodyObj.type == "offer") logModule.info(`${peerName} (${peerType}${peerId}) connects to ${renderId}`);
        }
      }
    }
  ...
  router.get("/sign_out", async (req, res) => {
    let peerId = req.query.peer_id;
    logModule.info(`/sign_out : ${peerId}`);
    const exist = await client.exists(peerId);
    ...
    try {
      ...
      if (peerType == "broadcaster" || peerType == "player") {
        ...
        logModule.info(`LOGOUT SUCCESS  : ${userId}`);
      }
      ...
      logModule.info(`${peerId} and data are removed (sign_out)`);
    } catch (error) {
      logModule.error(`sign_out update peer ERR : ${error}`);
    }
  ...
  router.get("/heartbeat", async (req, res) => {
    ...
    if (!peerIdData) {
      logModule.info(`HEARTBEAT 400 : ${peerId}'s data is deleted`);
      return res.status(400).end();
    }
    ...
    if (peerType == "broadcaster") {
      ...
      if (mobileXVersion || mobileXClientVersion) {
        if (reconId && status == "off-air" && reconReady == "true") {
          ...
          logModule.info(`{ type: "server_info", recon_info : { ip: ${reconIp}, port: ${broadcasterPort} }, audio_info : { ip: 127.0.0.1, port: 20036 } }`);
        }
      } else {
        if (reconId && renderId && status == "off-air" && reconReady == "true" && renderReady == "true") {
          ...
          logModule.info(`{ type: "server_info", recon_info : { ip: ${reconIp}, port: ${broadcasterPort} }, audio_info : { ip: ${reconIp}, port: ${audioListenPort} } }`);
        }
      }
    }
    ...
    if (refreshResult == "REMOVED") {
      logModule.info(`HEARTBEAT 400 : ${peerId}'s refresh result is REMOVED`);
      res.status(400).end();
...
async function checkRenderRestart(renderId) {
  ...
  logModule.info(`checkRenderRestart : ${renderId}`);

  router.peerList.removePeer(renderPeerId);
  await client.del(renderPeerId);

  logModule.info(`${renderPeerId} and data are removed (checkRenderRestart)`);
  ...
  if (mobileXVersion || mobileXClientVersion) {
    ...
    if (userId && userId != "null") {
      ...
      let broadcastStatus = await client.hget(userPeerId, "status");
      logModule.info(`checkRenderRestart : ${userId}(${userPeerId}) - ${broadcastStatus}`);
...
async function broadcastCancel(userId) {
  ...
  let peerIdData = await client.hgetall(peerId);
  logModule.info(`broadcastCancel - ${userId}(${peerId})`);

  if (peerIdData) {
    let reconId = peerIdData.reconId;
    let renderId = peerIdData.renderId;
    logModule.debug(`reconId : ${reconId}, renderId : ${renderId}`);
      ...
      if (reconId && reconId != "null") {
        ...
        if (renderIdArr && renderIdArr.length != 0) {
          for (let i = 0; i < renderIdArr.length; i++) {
            ...
            router.peerList.removePeer(renderPeerId);
            await client.del(renderPeerId);
            logModule.info(`${renderPeerId} and data are removed (broadcastCancel)`);
            rrModule.stopRender(renderId);
          }
      ...
      if (reconId && reconId != "null" && renderId && renderId != "null") {
        ...
        if (testVersion) {
          ...
          if (renderIdArr.length > 1) {
            for (let i = 1; i < renderIdArr.length; i++) {
              try {
                let testRenderId = renderIdArr[i];
                router.peerList.removePeer(testRenderId);
                await client.del(testRenderId);
                logModule.info(`${testRenderId} is removed (broadcastCancel)`);
                rrModule.stopRender(testRenderId);
              } catch (error) {
                logModule.error(`broadcastCancel update peer ERR - ${testRenderId} : ${error}`);
              }
            }
          }
        }
    ...
    try {
      ...
      logModule.error(`broadcastCancel update peer ERR - ${peerId} : ${error}`);
    } finally {
      if (!isLoggedIn[userId]) {
        await client.del(peerId);
        logModule.info(`${peerId}'s data is removed (broadcastCancel)`);
      }
    }
  }
}
